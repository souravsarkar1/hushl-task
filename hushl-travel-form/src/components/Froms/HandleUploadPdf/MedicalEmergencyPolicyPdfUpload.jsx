...
const HandleUploadPdf1 = ({ setFormData, formData, id }) => {
  const [loader, setLoader] = useState(false);
  const [loaderRemove, setLoaderRemove] = useState(Array(formData.healthSafety.medicalEmergencyPolicy?.length).fill(false));
      ...
      setFormData((prevData) => ({
        ...
        healthSafety: {
          ...
          medicalEmergencyPolicy: [
            ...prevData.healthSafety.medicalEmergencyPolicy.slice(0, index),
            { title: '', url: pdfUrl },
            ...prevData.healthSafety.medicalEmergencyPolicy.slice(index + 1),
          ],
        },
      }));
    } catch (error) {
  ...
  const handleRemoveImgUrl = async (index) => {
    ...
    try {
      ...
      setFormData((prevData) => ({
        ...
        healthSafety: {
          ...prevData.healthSafety,
          medicalEmergencyPolicy: prevData.healthSafety.medicalEmergencyPolicy.filter((_, i) => i !== index),
        },
      }));
    } catch (error) {
  ...
  const handleAddImgUrl = () => {
    ...
    setTimeout(() => {
      setFormData((prevData) => ({
        ...
        healthSafety: {
          ...prevData.healthSafety,
          medicalEmergencyPolicy: [...prevData.healthSafety.medicalEmergencyPolicy, { title: '', url: '' }],
        },
      }));
      setLoader(false);
    }, 1000);
  ...
  const isAddButtonDisabled =
    formData.healthSafety.medicalEmergencyPolicy?.some((medicalEmergencyPolicy) => medicalEmergencyPolicy.url === '' || medicalEmergencyPolicy.title === '');

  const isRemoveButtonDisabled = formData.healthSafety.medicalEmergencyPolicy?.length === 1;
  
  return (
    <div>
      <FormControl id="medicalEmergencyPolicy">
        <FormLabel>Medical Emergency Policy URLs</FormLabel>
          ...
          <Box p={2} key={index} gap={3} display="flex" alignItems="center">
            ...
            <FormControl id="title">
              ...
              <Input
                ...
                value={imgUrl.title}
                onChange={(e) => handleChangeMediaTitle(e, 'medicalEmergencyPolicy', index)}
                isRequired={true}
              />
            </FormControl>
...
export default HandleUploadPdf1;
