...
const NewsApi = () => {
  ...
  const handleSubmit = e => {
    ...
    setLoading(true);
    axios
      .post('http://127.0.0.1:8000/news/', { query_params: query_params })
      .then(res => {
        if (res && res.data) {
    ...
    <Box>
      ...
      <Box style={{ border: '1px solid teal', margin: '20px' }}>
        <Center>
              ...
              <Tbody>
                ...
                {fields.map((field, fieldIndex) => (
                  <Tr key={fieldIndex}>
                    <Th>{field.label}</Th>
                    {Object.keys(showdata).map((hotel, index) => (
                      <Td key={index}>{showdata[hotel][field.key]}</Td>
                    ))}
                  </Tr>
                ))}
              </Tbody>
          ...
          {!loading && (Object.keys(showdata).length === 0 || showdata === null || Object.keys(showdata) === null) && (
            <Center style={{ fontSize: '30px', color: 'teal' }}>Data Not found 😔</Center>
          )}
        </Center>
      </Box>
    </Box>
